            ...
            if (event.getType().equals(RenderGameOverlayEvent.ElementType.ALL)) {
                if (renderEntireBar) {
                    ...
                    for (int i1 = 0; i1 < 9; ++i1) {
                        ...
                        renderHotbarItem(matrixStack, j1, k1, event.getPartialTicks(), inventory.get(i1), itemRenderer,
                                fontRenderer);
                        renderHotbarItem(matrixStack, j1, k1 - 22, event.getPartialTicks(), inventory.get(i1 + 27),
                                itemRenderer, fontRenderer);
                        renderHotbarItem(matrixStack, j1, k1 - 44, event.getPartialTicks(), inventory.get(i1 + 18),
                                itemRenderer, fontRenderer);
                        renderHotbarItem(matrixStack, j1, k1 - 66, event.getPartialTicks(), inventory.get(i1 + 9),
                                itemRenderer, fontRenderer);
                    }
                    matrixStack.pushPose();
                    RenderSystem.setShaderColor(1F, 1F, 1F, 1.0F);
                    ...
                    for (int i = 27, j = 22; i > 0; i -= 9, j += 22) {
                        ...
                        renderHotbarItem(matrixStack, j1, k1 - j, event.getPartialTicks(), stack, itemRenderer,
                                fontRenderer);
                    }
...
//		int i = 182;
    ...
    private static void renderSelectedItem(PoseStack matrixStack, Minecraft mc, int scaledWidth, int scaledHeight,
        ...
        if (remainingHighlightTicks > 0 && !highlightingItemStack.isEmpty()) {
            MutableComponent mutablecomponent = (new TextComponent("")).append(highlightingItemStack.getHoverName()).withStyle(highlightingItemStack.getRarity().color);
