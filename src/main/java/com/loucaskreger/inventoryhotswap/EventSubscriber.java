			...
			if (event.getType().equals(RenderGameOverlayEvent.ElementType.ALL)) {
				if (renderEntireBar) {
					...
					for (int k = 3; k > 0; k--) {
						...
						fontRenderer.drawString(matrixStack, String.valueOf(k), width - 98,
					...
					for (int k = 3; k > 0; k--) {
						int l = ClientConfig.inverted.get() ? Math.abs(k - 3) + 1 : k;
						fontRenderer.drawString(matrixStack, String.valueOf(k),
...
//	}
	...
	public static void renderHorseJumpBar(MatrixStack matrixStack, int x, int scaledHeight) {
...
//		int i = 182;
		...
		mc.ingameGUI.blit(matrixStack, x, k, 0, 84, 182, 5);
		if (j > 0) {
			mc.ingameGUI.blit(matrixStack, x, k, 0, 89, j, 5);
		}

		mc.getProfiler().endSection();
	}

	private static void renderVehicleHealth(MatrixStack matrixStack, int scaledHeight, int scaledWidth) {
			...
			if (i != 0) {
				...
				for (boolean flag = false; i > 0; j1 += 20) {
					...
					for (int l1 = 0; l1 < k1; ++l1) {
						...
						int j2 = 0;
						int k2 = l - l1 * 8 - 9;
						mc.ingameGUI.blit(matrixStack, k2, i1, 52 + j2 * 9, 9, 9, 9);
						if (l1 * 2 + 1 + j1 < j) {
							mc.ingameGUI.blit(matrixStack, k2, i1, 88, 9, 9, 9);
						}

						if (l1 * 2 + 1 + j1 == j) {
							mc.ingameGUI.blit(matrixStack, k2, i1, 97, 9, 9, 9);
						}
					}
	...
	public static void renderExpBar(MatrixStack matrixStack, int x) {
		if (isGuiPushed) {
			...
			if (i > 0) {
				...
				mc.ingameGUI.blit(matrixStack, x, l, 0, 64, j, 5);
				if (k > 0) {
					mc.ingameGUI.blit(matrixStack, x, l, 0, 69, k, 5);
				}
			}
			...
			if (mc.player.experienceLevel > 0) {
				...
				mc.fontRenderer.drawString(matrixStack, s, (float) (i1 + 1), (float) j1, 0);
				mc.fontRenderer.drawString(matrixStack, s, (float) (i1 - 1), (float) j1, 0);
				mc.fontRenderer.drawString(matrixStack, s, (float) i1, (float) (j1 + 1), 0);
				mc.fontRenderer.drawString(matrixStack, s, (float) i1, (float) (j1 - 1), 0);
				mc.fontRenderer.drawString(matrixStack, s, (float) i1, (float) j1, 8453920);
			}
		}
	}
