	...
	public static void onGUIRender(final RenderGameOverlayEvent.Post event) {
		...
		if (wasKeyDown) {
			...
			if (event.getType().equals(RenderGameOverlayEvent.ElementType.ALL)) {
				...
				gui.blit(width - 91 + (currentIndex * (WIDTH - 2)), scaledHeight - WIDTH - HEIGHT, 0, 0, WIDTH, HEIGHT);
				...
				gui.blit(width - 92 + (currentIndex * (WIDTH - 2)), scaledHeight - WIDTH - HEIGHT + scrollFunc(), 0, 22,
						24, 24);
